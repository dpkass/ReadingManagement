...
public class EntryBuilder {
    ...
    private void chooseStatus() {
        if (rs == ReadingStatus.Waiting && waituntil == null) rs = ReadingStatus.Reading;      // autonullify paused when date arrives
        ...
        if (rt == 0) {
            ...
            rs = ReadingStatus.Started;
        } else rs = ReadingStatus.Reading;
    }
    ...
    public Book toEntry() {
        chooseStatus();
        return new Book(name, readto, link, rating, storyrating, charactersrating, drawingrating, ws, rs, lastread, waituntil, genres, booktype);
    }
}
